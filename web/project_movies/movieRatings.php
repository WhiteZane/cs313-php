...
<html>
    ...
    <body>
        ...
        <main>
            <?php 
                $checkRating = 'SELECT rental.rentalid, rental.movietitle, rental.description, rental.borrowed, rental.owner, genre.genrecategory, ratings.rating
                                                    FROM rental
                                                    INNER JOIN genre on rental.genre_fk=genre.genreid
                                                    INNER JOIN ratings on rental.rating_fk=ratings.ratingid
                                                    Where rental.rating_fk = :rating';  
            $statement = $db->prepare($checkRating);
            $statement->bindValue(':rating', $rateMovie, PDO::PARAM_INT );
            $statement->execute();
